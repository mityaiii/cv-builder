...
const DrawerButton = ({ state, promise, className }: { state: boolean, promise: (e: React.MouseEvent<HTMLElement>) => void, className?: string }) => {
  return (
    <button className={`transition liner delay-100 rounded-r-xl w-8 h-20 ${ bgColors.second_bg__color } ${ className } hover:shadow-[3px_0px_5px_2px_rgba(8,_112,_184,_0.2)]`} onClick={ promise }>
      { 
        state ? 
